...
class PostController extends BaseController
{
    ...
    public function store(BlogPostCreateRequest $request)
    {
        ...
        if ($request->hasFile('post_image')) {
            ...
            $imageInstance = Image::make($image)->resize(800, null, function ($constraint) { $constraint->aspectRatio(); $constraint->upsize(); });
            $imagePreview = Image::make($image)->resize(80, null, function ($constraint) { $constraint->aspectRatio(); $constraint->upsize(); });
    ...
    public function update(BlogPostUpdateRequest $request, $id)
    {
        ...
        if ($request->hasFile('post_image')) {
            ...
            $image = $request->file('post_image');
            $filename = Str::random(20) . '.' . $image->getClientOriginalExtension();
            $imageInstance = Image::make($image)->resize(1600, null, function ($constraint) { $constraint->aspectRatio(); $constraint->upsize(); });
            Storage::disk('public')->put('test/' . $filename, $imageInstance->stream());
            $imagePreviewInstance = Image::make($image)->resize(800, null, function ($constraint) { $constraint->aspectRatio(); $constraint->upsize(); });
